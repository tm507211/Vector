...
class Vector4{
 public:
  ...
  Vector4(const T& x, const T& y, const T& z, const T& w = T()):x(x), y(y), z(z), w(w){}
  Vector4(const Vector2<T>& v1, const Vector2<T>& v2):x(v1.x), y(v1.y), z(v2.x), w(v2.y){}
  Vector4(const Vector2<T>& v, const T& z = T(), const T& w = T()):x(v.x), y(v.y), z(z), w(w){}
  Vector4(const Vector3<T>& v, const T& w = T()):x(v.x), y(v.y), z(v.z), w(w){}
...
Vector4<T> operator * (T s, const Vector4<T>& v){
  return Vector3<T>(s * v.x, s * v.y, s * v.z, s * v.w);
}
...
/*
template <class T>
T cos(const Vector4<T>& v1, const Vector3<T>& v4){
  return dot(v1, v2) / v1.length() / v2.length();
}
template <class T>
T sin(const Vector3<T>& v1, const Vector3<T>& v2){
